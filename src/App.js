...
function calculateResults(incomingData) {
  ...
  const pointsPerTransaction = incomingData
  .map(transaction=> {
    let points = 0;
    //transactino 75
    let over100 = transaction.amt - 100;// -25
    
    if(over100 > 0){
      points += (over100 *2);
      points += 50;
    }
    else if(transaction.amt > 50 && transaction.amt <= 100){
  ...
  pointsPerTransaction.forEach(pointsPerTransaction => {
    ...
    if (noCustomerID(byCustomer, custid)) {
      byCustomer[custid] = [];      
    }    
    if (noTotalPointsByCustomerName(totalPointsByCustomer, name)) {
...
function noTotalPointsByCustomerName(totalPointsByCustomer, name) {
  return !totalPointsByCustomer[name];
}

function noCustomerID(byCustomer, custid) {
    ...
    <div>      
      ...
      <div className="container">
        <div className="row">
          ...
          <marquee behaviour="scroll" direction="right"><h2>Reward By Customer months </h2></marquee>
          </div>
        </div>
        <div className="row">
          <div className="col-8">
        ...
        <div className="container">    
          <div className="row">
            <div className="col-10">
              <marquee behaviour="scroll" direction="right"><h2>Reward By Customer </h2></marquee>
            </div>
          </div>      
          <div className="row">
            <div className="col-8">
